...
class CourtLineDetector:
    ...
    def draw_keypoint(self, image, keypoints):
        for i in range(0, len(keypoints), 2):
            x = int(keypoints[i])
            y = int(keypoints[i+1])
            cv2.putText(image, str(i/2), (x+80, y+10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 2)
            cv2.circle(image, (x + 80,y+20), 5, (0,0,255), -1)

        return image
    
    def draw_keypoints_on_video(self, video_frame, keypoints):
        output_video_frames = []
